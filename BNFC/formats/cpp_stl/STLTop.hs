...
makeSTL :: Bool -> Bool -> String -> FilePath -> IO ()
makeSTL make linenumbers name file = do
  (cf, isOK) <- tryReadCF file
  if isOK then do 
    let (hfile, cfile) = cf2CPPAbs linenumbers name cf
    ...
    let bison = cf2Bison linenumbers name cf env
